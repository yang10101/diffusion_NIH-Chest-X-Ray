...
class DMTrainer(Trainer):
    ...
    def train_in_iter(self):
        ...
        step_losses = []  # 记录这一轮中每个step的loss
        # 梯度累计步数
        accumulation_steps = 4  # 可根据需要调整此值
        ...
        for i, (images, labels) in enumerate(self.pbar):
            ...
            with autocast(enabled=self.amp):
                ...
                # Calculate the MSE loss and scale for accumulation
                loss = self.loss_func(noise, predicted_noise) * loss_scale
            ...
            # 只在累积指定步数后更新模型参数
            if (i + 1) % accumulation_steps == 0 or (i + 1) == len(self.pbar):
                # 优化器步骤（更新参数）
            ...
            # TensorBoard logging (记录未缩放的损失值)
            actual_loss = loss.item() * accumulation_steps
            self.pbar.set_postfix(MSE=actual_loss)
            self.tb_logger.add_scalar(tag=f"[{self.device}]: MSE", scalar_value=actual_loss,
        ...
        self.step_losses.extend(step_losses)  # 保存所有step的loss
        
        # TensorBoard记录每轮平均损失
        self.tb_logger.add_scalar(tag=f"[{self.device}]: Loss", scalar_value=avg_loss, global_step=self.epoch)
    ...
    def _generate_loss_curves(self):
        """
        生成训练损失曲线图
        """
        import matplotlib.pyplot as plt
        
        # 确保matplotlib可用于非交互环境
        plt.switch_backend('agg')
        
        # 1. 生成每个epoch的损失曲线
        plt.figure(figsize=(10, 6))
        plt.plot(range(len(self.epoch_losses)), self.epoch_losses, marker='o', linestyle='-')
        plt.title('Training Loss per Epoch')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.grid(True)
        plt.savefig(os.path.join(self.results_vis_dir, 'epoch_loss_curve.png'))
        plt.close()
        
        # 2. 生成每个step的损失曲线
        plt.figure(figsize=(12, 6))
        plt.plot(range(len(self.step_losses)), self.step_losses, linestyle='-', alpha=0.7)
        # 添加平滑曲线，提高可读性
        from scipy.ndimage import gaussian_filter1d
        if len(self.step_losses) > 10:  # 只有足够多的点才平滑
            smoothed = gaussian_filter1d(self.step_losses, sigma=5)
            plt.plot(range(len(self.step_losses)), smoothed, 'r-', linewidth=2)
            plt.legend(['Raw Loss', 'Smoothed Loss'])
        plt.title('Training Loss per Step')
        plt.xlabel('Step')
        plt.ylabel('Loss')
        plt.grid(True)
        plt.savefig(os.path.join(self.results_vis_dir, 'step_loss_curve.png'))
        plt.close()
        
        # 3. 保存数值数据，便于后续分析
        np.save(os.path.join(self.results_dir, 'epoch_losses.npy'), np.array(self.epoch_losses))
        np.save(os.path.join(self.results_dir, 'step_losses.npy'), np.array(self.step_losses))
        
        logger.info(msg=f"[{self.device}]: Loss curves generated and saved in {self.results_vis_dir}")